...
import { ANALYST_STATUS_LABELS } from "@/lib/constants/workflow-copy";

const statusConfig: Record<string, { label: string; color: string; bg: string }> = {
  analysis_done: { label: ANALYST_STATUS_LABELS.analysis_done, color: "text-indigo-600", bg: "bg-indigo-50" },
  reporting: { label: ANALYST_STATUS_LABELS.reporting, color: "text-cyan-600", bg: "bg-cyan-50" },
  completed: { label: ANALYST_STATUS_LABELS.completed, color: "text-emerald-600", bg: "bg-emerald-50" },
};
...
export default function AnalystHistoryPage() {
  ...
  const loadHistory = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const finishedJobs = (result.items || []).filter((job: any) =>
    ...
    <div className="p-4 md:p-10 space-y-6 pb-24 md:pb-10 max-w-5xl mx-auto animate-in fade-in duration-500">
      <div className="flex justify-between items-center px-1">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 text-[9px] font-bold uppercase tracking-widest mt-1">Arsip pengerjaan pasca analisis</p>
          </div>
        </div>
        <button
        ...
        >
          <RefreshCw className={cn("h-3 w-3", refreshing && "animate-spin")} /> {refreshing ? "Memperbarui..." : "Refresh"}
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center gap-4 shadow-sm border-b-4 border-b-emerald-500">
          ...
          <div><p className="text-xl font-black text-slate-900 leading-none">{jobs.length}</p><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-1">Pasca Analisis</p></div>
        </div>
        <div className="bg-white p-4 rounded-2xl border border-slate-100 flex items-center gap-4 shadow-sm border-b-4 border-b-indigo-500">
          ...
          <div><p className="text-xl font-black text-slate-900 leading-none">{jobs.filter((job) => job.status === "reporting").length}</p><p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-1">Sedang Di Reporting</p></div>
        </div>
      </div>
                      ...
                      <div className="flex-1 space-y-3">
                        ...
                        <div>
                          <h3 className="font-black text-slate-800 text-[15px] leading-tight group-hover:text-indigo-600 transition-colors">
                            {item.quotation?.profile?.company_name || item.quotation?.profile?.full_name || "Klien Perorangan"}
                          </h3>
